...
from ama_xiv_combat_sim.simulator.game_data.patch_655.convenience_timings import get_auto_timing
...
def add_blm_skills(skill_library):
  ...
  instant_timing_spec = TimingSpec(
      base_cast_time=0, animation_lock=blm_caster_tax_ms)
  skill_library.set_current_job_class('BLM')

  skill_library.set_status_effect_priority(('Swiftcast', 'Triplecast'))
  skill_library.add_resource(name="Astral Fire", job_resource_settings=JobResourceSettings(max_value=3, expiry_from_last_gain=15*1000, skill_allowlist=(
      'Blizzard', 'Transpose', 'Fire', 'Fire III', 'Fire IV', 'Blizzard IV', 'Blizzard III', 'Flare', 'Despair', 'High Fire II', 'High Blizzard II', 'Paradox')))
  skill_library.add_resource(name="Umbral Ice", job_resource_settings=JobResourceSettings(max_value=3, expiry_from_last_gain=15*1000, skill_allowlist=(
      'Blizzard', 'Transpose', 'Fire', 'Fire III', 'Fire IV', 'Blizzard IV', 'Blizzard III', 'Flare', 'Despair', 'High Fire II', 'High Blizzard II', 'Paradox')))
  ...
  def get_enochian_damage_spec_cross(base_potency, is_fire_spell):
    res = {}
    elem_strs = ['1 Astral Fire', '2 Astral Fire', '3 Astral Fire',
                 '1 Umbral Ice', '2 Umbral Ice', '3 Umbral Ice']
    fire_potency_modifiers = {'1 Astral Fire': 1.4, '2 Astral Fire': 1.6,
                              '3 Astral Fire': 1.8, '1 Umbral Ice': 0.9, '2 Umbral Ice': 0.8, '3 Umbral Ice': 0.7}
    ice_potency_modifiers = {'1 Astral Fire': 0.9, '2 Astral Fire': 0.8,
                             '3 Astral Fire': 0.7, '1 Umbral Ice': 1, '2 Umbral Ice': 1, '3 Umbral Ice': 1}

    res[SimConsts.DEFAULT_CONDITION] = DamageSpec(
        potency=base_potency)  # no astral/umbral fire
  ...
  def get_enochian_timing_spec_cross(base_cast_time, is_fire_spell, application_delay=0):
    ...
    elem_strs = ['1 Astral Fire', '2 Astral Fire', '3 Astral Fire',
                 '1 Umbral Ice', '2 Umbral Ice', '3 Umbral Ice']
    fire_cast_time_modifiers = {'1 Astral Fire': 1, '2 Astral Fire': 1,
                                '3 Astral Fire': 1, '1 Umbral Ice': 1, '2 Umbral Ice': 1, '3 Umbral Ice': 0.5}
    ice_cast_time_modifiers = {'1 Astral Fire': 1, '2 Astral Fire': 1,
                               '3 Astral Fire': 0.5, '1 Umbral Ice': 1, '2 Umbral Ice': 1, '3 Umbral Ice': 1}

    res[SimConsts.DEFAULT_CONDITION] = TimingSpec(
        base_cast_time=base_cast_time, animation_lock=blm_caster_tax_ms, application_delay=application_delay)
    for elem_str in elem_strs:
      cast_modifier = fire_cast_time_modifiers[elem_str] if is_fire_spell else ice_cast_time_modifiers[elem_str]
      res[elem_str] = TimingSpec(base_cast_time=int(cast_modifier*base_cast_time),
                                 animation_lock=blm_caster_tax_ms, application_delay=application_delay)
    return res
  ...
  for k in fire_keys:
    assembled_str = 'Sharpcast' if k == SimConsts.DEFAULT_CONDITION else '{}, Sharpcast'.format(
        k)
  ...
  for k in fire_iii_keys:
    assembled_str = 'Firestarter' if k == SimConsts.DEFAULT_CONDITION else '{}, Firestarter'.format(
        k)
  ...
  skill_library.add_skill(Skill(name='Fire III',
                                ...
                                job_resource_spec=(clear_umbral_ice, JobResourceSpec(
                                    name='Astral Fire', change=+3, refreshes_duration_of_last_gained=True)),
                                follow_up_skills=(enochian_buff_follow_up,)))
  skill_library.add_skill(Skill(name='Blizzard III',
                                ...
                                job_resource_spec=(clear_astral_fire, JobResourceSpec(
  ...
  flare_elem_mults = {'1 Astral Fire': 1.4, '2 Astral Fire': 1.6,
                      '3 Astral Fire': 1.8, '1 Umbral Ice': 0.9, '2 Umbral Ice': 0.8, '3 Umbral Ice': 0.7}
