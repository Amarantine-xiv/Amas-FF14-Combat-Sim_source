...
class Utils:
  ...
  def get_best_key(keys, with_condition):
    ...
    assert len(best_keys) == 1, 'Did not find exactly one match. matches:{}, Keys: {}, with_condition {}'.format(best_keys, keys, with_condition)
    return best_keys[0]
  ...
  def get_positional_condition(skill, init_skill_modifier):
    ...
    if not missed_positional_damage_spec or init_skill_modifier.bonus_percent is None:
    ...
    skill_modifier_no_combo_no_pos = copy.deepcopy(skill_modifier).add_to_condition('No Combo, No Positional')
    min_potency_key = Utils.get_best_key(damage_spec_to_use.keys(), skill_modifier_no_combo_no_pos.with_condition)
    min_potency = damage_spec_to_use[min_potency_key].potency

    skill_modifier_no_combo_no_pos = copy.deepcopy(skill_modifier).add_to_condition('No Combo, No Positional')
    skill_modifier_no_combo = copy.deepcopy(skill_modifier).add_to_condition('No Combo')
    skill_modifier_no_pos = copy.deepcopy(skill_modifier).add_to_condition('No Positional')

    for to_try in [skill_modifier, skill_modifier_no_pos, skill_modifier_no_combo, skill_modifier_no_combo_no_pos]:
        potency_key = Utils.get_best_key(damage_spec_to_use.keys(), to_try.with_condition)
        potency = damage_spec_to_use[potency_key].potency
        potential_bonus_potency = int(100 * (potency - min_potency) / potency)
    ...
    raise ValueError(
        "Could not find match for skill {} with bonusPercent {} and condition {}. Please contact a dev.\n".format(
            skill.name, init_skill_modifier.bonus_percent, init_skill_modifier.with_condition))
