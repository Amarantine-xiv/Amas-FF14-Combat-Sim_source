...
class TestStatusEffectTracker(TestClass):
  ...
  def conditional_addition_with_num_uses_and_allowlist(self):
    ...
    for i in range(0, len(skills_to_use)):
      ...
      result = (se.compile_buffs(t=0, skill=skills_to_use[i]), se.compile_debuffs(t=0, skill=skills_to_use[i]))
  ...
  def status_effect_priority(self):
    ...
    result = (se.compile_buffs(1000, skill=test_instant_gcd), se.compile_debuffs(1000, skill=test_instant_gcd))
