...
class SnapshotAndApplicationEvents:
  ...
  class Event(namedtuple('Event', ['priority', 'event_times', 'skill', 'skill_modifier', 'snapshot_status', 'event_id', 'job_resources_added', 'job_conditional_processed', 'combo_conditional_processed'])):
    def __lt__(self, other):
  ...
  @staticmethod
  def __create_event(priority, primary_time, secondary_time, skill, skill_modifier, snapshot_status, event_id, job_resources_added, job_conditional_processed, combo_conditional_processed):
  ...
  def add(self, priority, primary_time, secondary_time, skill, skill_modifier, snapshot_status, event_id=None, job_resources_added=False, job_conditional_processed=False, combo_conditional_processed=False):
    if event_id is None:
    ...
    heapq.heappush(self.__q, self.__create_event(priority, primary_time, secondary_time, skill, skill_modifier, snapshot_status, event_id, job_resources_added, job_conditional_processed, combo_conditional_processed))

  def get_first_damage_time(self):
    first_damage_time = None
    for (_,event_times,skill,_,_,_,_,_,_) in self.__q:
  ...
  def get_last_event_time(self):
    ...
    for (_,event_times,_,_,_,_,_,_,_) in self.__q:
      curr_time = event_times.primary if event_times.secondary is None else event_times.secondary
